...
--
-- Host: localhost
-- Czas wygenerowania: 18 Sty 2014, 14:37
...
--
CREATE DATABASE IF NOT EXISTS `windykacja` DEFAULT CHARACTER SET utf8 COLLATE utf8_polish_ci;
USE `windykacja`;
...
CREATE TABLE `sprawy` (
  ...
  `data_zajecia` date DEFAULT NULL,
  `skladnik_majatkowy` enum('wynagrodzenie za pracę','świadczenie emerytalno-rentowe','rachunek bankowy','wierzytelność pieniężna') NOT NULL,
...
CREATE TABLE `wierzyciel` (
  ...
  `typ_stopy_procentowej` enum('referencyjna','podatkowa','lombardowa','stopa_z_wyroku') NOT NULL,
...
CREATE TABLE `wierzyciele_sprawy` (
  ...
  `tytul_wykonawczy` varchar(50) NOT NULL,
  `data_tytulu` date DEFAULT NULL,
  `tytul_wydanyPrzez` varchar(40) NOT NULL,
  `data_ZUS` date DEFAULT NULL,
  `id_organu_egzekucyjnego` int(11) DEFAULT NULL,
